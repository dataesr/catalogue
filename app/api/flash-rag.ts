...
async function queryFlashRag(q: string, source: string, top_k: number): Promise<RagResponse> {
  const { data, error } = await api.rag.get({ query: { q, source, top_k } })
...
export function useFlashRag(q: string, source: string, top_k: number) {
  return useQuery({
    queryKey: ["flash-rag", q, source, top_k],
    queryFn: () => queryFlashRag(q, source, top_k),
    enabled: q.length > 0,
